...
* デフォルト15分間隔（設定可能: 5分〜30分）で自動的にデータ取得処理をバックグラウンドで走らせ、表示を更新する。
...
* **トークン消費の抑制**: `GetUserStatus` API 自体の呼び出しはアカウントのクォータ（Work Done）を消費しない前提とするが、Language Server への過剰な負荷を避けるため、自動更新の最小間隔は5分とする。
